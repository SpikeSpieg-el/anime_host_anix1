import { Lock, Users, Sword, History, Sparkles, Mail, Calendar, Gift, Lightbulb, Brain, Newspaper, PenTool, BarChart3, Filter } from "lucide-react"
...
export const tutorialSections: TutorialSection[] = [
  {
    ...
    steps: [
      ...
      { title: "Войдите", detail: "После успешной авторизации вы увидите Dashboard с 11 вкладками: Users, PvP, AI Battle, Battle Logs, Карты, Рассылка, События, Новости, Редакция, Туториал, Аналитика" },
    ],
    warnings: [
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Фильтр по активности", detail: "Выпадающий список фильтра: все пользователи, с историей просмотров, с закладками, с PvP-битвами или без активности" },
      { title: "Сортировка", detail: "Второй список управления порядком: по умолчанию, сначала активные, больше истории/закладок/PvP-битв, по имени (А→Я)" },
      { title: "Сброс фильтров", detail: "Кнопка «Сбросить» возвращает поиск, фильтр и сортировку в исходное состояние. Над списком показано «Показано X из Y пользователей»" },
    ...
    tips: [
      ...
      "Фильтр «Без активности» помогает быстро найти ботов и заброшенные аккаунты",
      "Комбинация поиска + фильтра + сортировки применяется одновременно",
    ],
  },
  {
    id: "pvp",
  ...
  {
    id: "ai_battle",
    title: "AI Battle — Баланс PvE и обучение AI",
    icon: <Brain className="w-5 h-5" />,
    description: "Метрики баланса локаций, профили обучения AI и сигналы антифарма",
    steps: [
      { title: "Откройте вкладку AI Battle", detail: "Данные загружаются автоматически; кнопка «Обновить данные» перезапрашивает метрики" },
      { title: "Сводка", detail: "Верхние карточки: всего PvE боёв, винрейт игроков (цель баланса 45–65%), средняя длительность в ходах, профили обучения" },
      { title: "Баланс локаций", detail: "Каждая локация показывает винрейт игрока/ИИ и статус: insufficient (мало данных), balanced, player_advantage, ai_advantage" },
      { title: "Профили фармеров (антифарм)", detail: "Список профилей с количеством боёв, побед, подряд побед и уровнем риска: normal / medium / high" },
      { title: "Learning Profiles", detail: "Профили обучения ИИ по игрокам — как часто и каким стилем они играют" },
    ],
    tips: [
      "Зелёные «Сбалансированные локации» — винрейт игроков в диапазоне 45–65%",
      "Если локация «insufficient» — нужно больше боёв, прежде чем делать выводы о балансе",
      "High risk профили проверяйте вручную: возможен полуавтоматический фарм",
    ],
    warnings: [
      "Не nerf'айте локацию только по нескольким боям — сначала наберите статистику",
    ],
  },
  {
    id: "cards",
  ...
  {
    id: "news",
    title: "Новости — Управление новостями сайта",
    icon: <Newspaper className="w-5 h-5" />,
    description: "Создание, редактирование и публикация новостей на главной",
    steps: [
      { title: "Откройте вкладку Новости", detail: "Список всех новостей загружается автоматически (до 100 записей)" },
      { title: "Создать новость", detail: "Нажмите «Создать новость»: заголовок и анонс обязательны, тело (body), картинка и автор — по желанию" },
      { title: "Предпросмотр", detail: "В форме создания и редактирования есть вкладки «edit» и «preview» — проверяйте разметку до публикации" },
      { title: "Опубликовать/скрыть", detail: "Глаз переключает is_published: опубликованные новости видны пользователям, скрытые — черновики" },
      { title: "Редактировать и удалить", detail: "Карандаш открывает форму редактирования, корзина удаляет новость (с подтверждением)" },
    ],
    tips: [
      "Анонс (excerpt) кратко показывается в списке — делайте его ёмким",
      "Новость с is_published=false можно подготовить заранее и опубликовать одной кнопкой",
    ],
    warnings: [
      "Удаление новости необратимо — сначала снимите с публикации, если может понадобиться вернуть",
    ],
  },
  {
    id: "editorial",
    title: "Редакция — Обзоры редакции",
    icon: <PenTool className="w-5 h-5" />,
    description: "Написание и обновление редакционных обзоров к аниме",
    steps: [
      { title: "Откройте вкладку Редакция", detail: "Список обзоров (editorial_reviews) загружается автоматически, свежие сверху" },
      { title: "Новый обзор", detail: "Укажите Anime ID (с Shikimori), текст обзора и автора (по умолчанию «Редакция Weebx»)" },
      { title: "Шаблоны (Cheat Sheet)", detail: "Кнопка шаблонов показывает готовые блоки структуры обзора — вставляются в текст одной кнопкой или копируются в буфер" },
      { title: "Предпросмотр", detail: "Тумблер preview показывает, как обзор отформатируется на странице аниме" },
      { title: "Редактирование", detail: "Существующий обзор открывается кнопкой редактирования; изменения сохраняются в ту же запись" },
    ],
    tips: [
      "Anime ID должен соответствовать аниме на сайте (Shikimori ID)",
      "Копируйте шаблоны в буфер, если хотите переиспользовать их во внешнем редакторе",
    ],
  },
  {
    id: "analytics",
    title: "Аналитика — Экспорт данных Umami",
    icon: <BarChart3 className="w-5 h-5" />,
    description: "Выгрузка событий и сессий аналитики в CSV",
    steps: [
      { title: "Откройте вкладку Аналитика", detail: "Форма экспорта данных самохостируемого Umami (analytics.weeb-x.com)" },
      { title: "Введите строку подключения", detail: "Укажите Postgres connection string базы Umami, например postgresql://user:pass@host:5432/umami" },
      { title: "Скачать CSV", detail: "Нажмите «Скачать CSV» — сервер объединяет события и сессии и отдаёт готовый файл" },
    ],
    tips: [
      "Строку подключения можно найти в переменных окружения инстанса Umami в Coolify",
      "Экспорт запускается через /api/admin/analytics-export и требует авторизованной админ-сессии",
    ],
    warnings: [
      "Не оставляйте строку подключения в общедоступных местах — это доступ к базе аналитики",
    ],
  },
]
