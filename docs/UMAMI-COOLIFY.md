...
| В профиле посетителя был только Distinct ID, без свойств; гости без аккаунта были анонимны и не различались | `identify` шлёт свойства (Session Data); гостям — стабильный `guest:<uuid>` id, см. §4.2 |
...
Гостевой `guest:<uuid>` (см. §4.2) — случайный UUID без персональных данных,
создаётся только после согласия; свойства identify (`user_type`, `first_visit`,
`landing_page`, `referrer_domain`, `device`, `guest_cards`, `guest_dust`) —
псевдонимные/агрегированные, email в них не попадает.
...
### 4.2 Session Data: свойства посетителей (вкладка «Свойства»)

`identifyUser(id, data)` (`lib/analytics.ts`) теперь всегда шлёт не только Distinct ID,
но и пары «ключ — значение», которые Umami сохраняет в `session_data` и показывает во
вкладке **«Свойства»** профиля посетителя. Эти же свойства доступны в **глобальных
фильтрах дашборда** (User Property / Session Data) — например, вся статистика
только по гостям или только по гостям, крутившим гачу.

**Кто есть кто.** `AnalyticsWrapper` идентифицирует посетителя до первого pageview:

| Посетитель | Distinct ID | Когда |
| --- | --- | --- |
| Авторизованный | `supabase:<uuid>` | сразу после восстановления/получения сессии |
| Гость (без аккаунта) | `guest:<uuid>` | сразу после согласия на аналитику |

`guest:<uuid>` — случайный UUID, созданный **один раз после согласия** и хранящийся
в localStorage (`weebx-guest-id`). До этого визиты без аккаунта были анонимными
(session ID из IP + UA + месячной соли), их нельзя было ни различить, ни вернуть
в Retention. Теперь у каждого гостя постоянный профиль: сессия Umami выводится
из `uuid(websiteId, distinctId)`, поэтому визиты одного гостя сшиваются в одного
«постоянного посетителя» (Retention, Journeys, список Visitors).

**Свойства гостя** (снимок на визит, в момент identify):

| Ключ | Значение | Зачем фильтровать |
| --- | --- | --- |
| `user_type` | `guest` | «кто без аккаунта» — базовый фильтр |
| `first_visit` | дата (YYYY-MM-DD) первого визита на устройстве | новые когорты гостев |
| `landing_page` | путь первого визита (`/gacha`, `/watch/…`) | откуда начал |
| `referrer_domain` | домен первого источника или `direct` | какие ссылки приводят |
| `device` | `mobile` / `tablet` / `desktop` | сегмент по устройствам |
| `guest_cards` | число карт в локальной гаче до регистрации | вовлечённость до аккаунта |
| `guest_dust` | пыль в локальной гаче до регистрации | вовлечённость до аккаунта |
| `guest_gacha_active` | `true`, если карты или пыль уже есть | «гости, пробовали гачу» |

**Свойства зарегистрированного:**

| Ключ | Значение |
| --- | --- |
| `user_type` | `registered` |
| `username` | ник; **не отправляется, если это email** (профиль по умолчанию создаётся из email) |
| `referred` | `true`, если аккаунт пришёл по реферальной ссылке |

Механика: Umami 3.0.3 **слепляет** `session_data` по ключу (новый identify
перезаписывает свой ключ, чужие не трогает), поэтому wrapper шлёт identify
повторно, когда догружается профиль: сначала `user_type=registered` без ника,
позже — с `username` (дедупликация по id+properties в `identifyUser` не даёт
спамить одинаковыми identify). Логин/логаут переключают Distinct ID; прошлые
гостевые данные не стираются — гость после logout снова `guest:<тот же uuid>`.

**Приватность:** в свойства уходят только псевдонимные/агрегированные данные
(свои UUID, даты, пути, домены, счётчики прогресса). Email, пароли, токены и
реквизиты не отправляются; `username` прогоняется через проверку на email.
Гостевой UUID создаётся строго после согласия, до согласия — не создаётся.

**Ограничение:** пересчёт guest → registered не сшивается задним числом (у Umami
нет aliasing API): воронки «до/после входа» смотрите отдельными, как и раньше
(см. §5 «Воронки»). Свойства гостя — снимок на визит: `guest_cards`/`guest_dust`
фиксируются в момент identify, а не в реальном времени.
