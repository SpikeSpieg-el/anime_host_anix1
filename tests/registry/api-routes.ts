...
export const API_ROUTES: ApiRegistryEntry[] = [
  ...
  { path: "/api/auth/captcha", methods: ["GET"], auth: "public", notes: "генерация капчи для защиты от ботов (HMAC-SHA256)" },
  ...
  { path: "/api/hero-recommendation", methods: ["GET"], auth: "bearer" },
  ...
  { path: "/api/recommendations/ai-generate", methods: ["POST"], auth: "bearer", notes: "AI подборка - только для авторизованных" },
  { path: "/api/recommendations/search-anime", methods: ["POST"], auth: "public" },
  { path: "/api/recommendations/user-data", methods: ["GET"], auth: "bearer", notes: "Данные пользователя для рекомендаций - только для авторизованных" },
