...
import { GridSkeleton, NewsSkeleton } from '@/components/shared/skeleton'
...
export function HomePageClient({ initialData }: HomePageClientProps) {
  ...
  const [loading, setLoading] = useState<Record<string, boolean>>(() => ({
    popularNow: initialData.popularNow.length === 0,
    popularAlways: initialData.popularAlways.length === 0,
    ongoingAnime: initialData.ongoingAnime.length === 0,
    newsUpdates: initialData.newsUpdates.length === 0,
    announcements: initialData.announcements.length === 0,
  }))

  useEffect(() => {
    const emptyFields = Object.entries(loading)
      .filter(([, v]) => v)
      .map(([k]) => k)
    if (emptyFields.length === 0) return

    const controller = new AbortController()
    fetch(`/api/home-data?fields=${emptyFields.join(",")}`, { signal: controller.signal })
      .then((r) => r.json())
      .then((result) => {
        setData((prev) => ({ ...prev, ...result }))
        setLoading((prev) => {
          const next = { ...prev }
          emptyFields.forEach((f) => { next[f] = false })
          return next
        })
      })
      .catch(() => {
  ...
  return (
    <>
      <main className="min-h-screen bg-background text-foreground pb-20 md:pb-24 selection:bg-primary/30 dark:bg-zinc-950 dark:text-white dark:selection:bg-orange-500/30">
        ...
        {/* 3. ЛЕНТА ОБНОВЛЕНИЙ */}
        {(data.ongoingAnime && data.ongoingAnime.length > 0) || (data.announcements && data.announcements.length > 0) ? (
        ...
        <section id="popular" className="mb-12 sm:mb-20">
          ...
          ) : data.popularNow && data.popularNow.length > 0 ? (
            <>
              <div className="grid grid-cols-2 gap-3 sm:hidden">
                {data.popularNow.slice(0, 12).map((anime) => (
        ...
        <section id="ongoing" className="mb-12 sm:mb-20">
          ...
          {loading.ongoingAnime ? (
            <GridSkeleton items={12} />
          ) : data.ongoingAnime && data.ongoingAnime.length > 0 ? (
            <>
              <div className="grid grid-cols-2 gap-3 sm:hidden">
        ...
        <section id="legendary" className="mb-14 sm:mb-20">
          ...
          {loading.popularAlways ? (
            <GridSkeleton items={12} />
          ) : data.popularAlways && data.popularAlways.length > 0 ? (
            <>
              <div className="grid grid-cols-2 gap-3 sm:hidden">
