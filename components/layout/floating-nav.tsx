...
export function FloatingNav({ config, variant = 'default' }: FloatingNavProps) {
  ...
  return (
    <div 
      className={cn(
        "fixed z-40 transition-all duration-500 ease-in-out flex flex-row lg:flex-col items-end gap-3",

        // --- ПОЛОЖЕНИЕ ---
        // Мобильные и планшеты: по центру внизу
        "left-1/2 -translate-x-1/2 bottom-6",
        // Десктоп: справа внизу, сброс центрирования
        "lg:left-auto lg:right-8 lg:translate-x-0 lg:bottom-8",
        ...
        // На мобилках и планшетах скрываем, когда скроллим ВВЕРХ (чтобы выплыл основной Navbar Dock)
        !isScrollingDown && "max-lg:translate-y-32 max-lg:opacity-0"
      )}
    >
      ...
      {showScrollUpButton && (
        <button
          ...
          className="hidden lg:flex w-12 h-12 rounded-[18px] bg-background/80 backdrop-blur-2xl border border-white/10 dark:border-white/5 shadow-2xl items-center justify-center text-muted-foreground hover:text-primary transition-all active:scale-90"
        >
          <ArrowUpIcon size={20} />
        </button>
      )}
      ...
      <nav className="flex flex-row lg:flex-col items-center gap-1.5 p-1.5 max-w-[calc(100vw-1rem)] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden bg-background/80 backdrop-blur-2xl border border-white/10 dark:border-white/5 rounded-[22px] shadow-2xl">
        {visibleNavItems.map((item) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative group flex items-center justify-center w-11 h-11 lg:w-12 lg:h-12 rounded-[18px] transition-all duration-300",
                isActive 
            ...
            >
              ...
              <span className="hidden lg:block absolute right-full mr-4 px-3 py-1.5 rounded-xl bg-popover/90 backdrop-blur-md border border-border text-xs font-semibold text-popover-foreground opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all pointer-events-none whitespace-nowrap shadow-xl">
                {item.label}
              </span>
            </button>
          )
        ...
        {showScrollUpButton && (
          <>
            <div className="w-[1px] h-6 bg-border/50 mx-1 lg:hidden" />
            <button
              onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
              className="lg:hidden flex items-center justify-center w-11 h-11 rounded-[18px] bg-primary/10 text-primary active:scale-90 transition-all"
            >
              <ArrowUpIcon size={20} />
            </button>
          </>
